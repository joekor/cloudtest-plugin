...
public class TransactionThreshold extends AbstractDescribableImpl<TransactionThreshold> {
	...
	private static final String THRESHOLD_TO_SCOMMAND_FORMAT = "%s" + THRESHOLD_STR_SEPERATOR + "%s"
			+ THRESHOLD_STR_SEPERATOR + "%s" + THRESHOLD_STR_SEPERATOR + "%s";
	 ...
	 *            - name of transaction
	 * @param thresholdname
	 *            - threshold name, picked from a list box
	 * @param thresholdmaxvalue
	 *            - the threshold value
	 * @param thresholdid
	 *            - a place holder for the threshold name, when it is needed to
	 *            be loaded by the UI
	 */
	@DataBoundConstructor
	public TransactionThreshold(String transactionname, String thresholdname, String thresholdminvalue,
			String thresholdmaxvalue, String thresholdid) {
		this.transactionname = transactionname;
		this.thresholdname = thresholdname;
		this.thresholdminvalue = thresholdminvalue;
		this.thresholdmaxvalue = thresholdmaxvalue;

		this.resultType = ResultType.getByResultType(thresholdname);
	}
	...
	public String toScommandString() {
		...
		String scommandFormatted = String.format(THRESHOLD_TO_SCOMMAND_FORMAT, transactionname, trimSpaceThresholdName,
				thresholdminvalue, thresholdmaxvalue);
	...
	public static class DescriptorImpl extends Descriptor<TransactionThreshold> {
		 ...
		 * 
		 * @param value
		 *            - the value from jenkins UI
		 * @return if "FormValidation.error" is returned, the user is notified
		 *         of bad input
		 */
		...
		/**
		 * checks the validity of the threshold value parameter it should be an
		 * alphanumeric number
		 * 
		 * @param value
		 *            - entered via jenkins UI
		 */
		...
		/**
		 * required by the jenkins framework fills the thresholds names into the
		 * listbox item. than the user can select one of them as the desired
		 * threshold name.
		 * 
		 * @return ListBoxModel item filled with the strings to be displayed in
		 *         the listbox
		 */
		public ListBoxModel doFillThresholdnameItems() {
			final ListBoxModel listBox = new ListBoxModel();

			for (ResultType x : ResultType.values()) {

				listBox.add(x.getUIText(), x.getResultType());
			}
