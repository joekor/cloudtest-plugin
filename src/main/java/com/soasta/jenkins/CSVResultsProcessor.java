...
public class CSVResultsProcessor {
	...
	public CSVResultsProcessor(String csvResults, List<TransactionThreshold> thresholds, PrintStream jenkinsLogger) {
				...
				for (TransactionThreshold threshold : thresholds) {
					...
					if (value != null) {
						...
						if (!validateTest(csvRecord, threshold)) {
							...
							testcase.setFailure("Transaction time " + value + " not within defined bounds of " + threshold.getThresholdminvalue() + " - " + threshold.getThresholdmaxvalue());
						}
